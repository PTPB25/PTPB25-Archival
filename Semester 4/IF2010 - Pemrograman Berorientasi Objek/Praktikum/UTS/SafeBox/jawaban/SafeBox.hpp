...
public:
    ...
    SafeBox(int cap, function<bool(const T&)> validator = nullptr)
    : cap(cap), validator(validator)
    {}
    ...
    void store(const T& item){
        if(validator != nullptr){
            if(!validator(item)) throw InvalidItemException<T>(item);
        }
        if((int)items.size() >= cap) throw BoxFullException(cap);
    ...
    T retrieve(){
        if(isEmpty()) throw BoxEmptyException();
        T last = peek();
        items.pop_back();
        return last;
    }
    ...
    friend ostream& operator<<(ostream& os, const SafeBox<T>& box) {
        ...
        if(!box.isEmpty()){
            auto printBox = box;
            printBox.reverse();
            while(!printBox.isEmpty()){
                os << printBox.retrieve();
                if(printBox.size() > 0) os << ", ";
            }
        }
        os << "]";
        return os;
