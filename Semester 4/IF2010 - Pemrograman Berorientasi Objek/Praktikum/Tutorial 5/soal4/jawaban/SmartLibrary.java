...
public class SmartLibrary<T extends Comparable<T>> implements Iterable<T>, Searchable<T> {
    ...
    */
    ...
    public void sortByRatingDesc() {
        ...
        Collections.sort(books, Comparator.comparingDouble(t -> ((Book) t).getRating()).reversed());
    }
    ...
    public T findByAuthor(String author) {
        ...
        Collections.sort(books, Comparator.comparing(t -> ((Book) t).getAuthor()));

        int index = Collections.binarySearch(books, (T) new Book("", author, "", 0, 0.0), Comparator.comparing(t -> ((Book) t).getAuthor()));
    ...
    public List<T> getTopRated(int n) {
        ...
        return books.stream().sorted(Comparator.comparingDouble(t -> ((Book) t).getRating()).reversed()).limit(n).collect(Collectors.toList());
    }
    ...
    public List<T> getByGenre(String genre) {
        // Apa hayo lanjutannya, lengkapin streamnya ya!
        return books.stream().filter(t -> ((Book) t).getGenre().equalsIgnoreCase(genre)).collect(Collectors.toList());
    }
